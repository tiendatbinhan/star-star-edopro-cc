...
function s.initial_effect(c)
    ...
    e4:SetCategory(CATEGORY_DEFCHANGE + CATEGORY_POSITION)
...
function s.addfilter(c)
    return c:IsSetCard(SET_STARSTAR) and c:IsType(TYPE_CONTINUOUS) and c:IsType(TYPE_SPELL) and c:IsAbleToHand()
end
...
function s.deftg(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
    ...
    if chk == 0 then
        return Duel.IsExistingTarget(s.deffilter, tp, 0, LOCATION_MZONE, 1, nil, tp)
    end
    Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_TARGET)
    local tc = Duel.SelectTarget(tp, s.deffilter, tp, 0, LOCATION_MZONE, 1, 1, nil, tp)
    Duel.SetTargetCard(tc)
    local amount = -s.defval(tc:GetFirst())
    Duel.SetTargetParam(amount)
    Duel.SetOperationInfo(0, CATEGORY_DEFCHANGE, tc, 0, 1-tp, amount)
